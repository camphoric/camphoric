/**
 * Registrations (SPEC §8.4). A sortable/filterable table of the event's
 * registrations (primary camper, type, email, owed, balance, payment status);
 * selecting one (URL-addressable via `?registrationId`) opens its editor.
 *
 * Invitations, registration types, payments/fees, admin-only attributes, and
 * camper reordering are separate concerns handled in later slices.
 */
...
export function EventAdminRegistrations() {
  ...
  const columns = useMemo<ColumnDef<AugmentedRegistration, unknown>[]>(
    () => [
      {
        id: 'camper',
        header: 'Primary camper',
        accessorFn: (r) => camperName(r.campers[0]),
      },
      {
        id: 'type',
        header: 'Type',
        accessorFn: (r) => r.registrationType?.label ?? '—',
      },
