...
import { RegistrationState } from "./RegisterPage";

export function calculatePrice(state: RegistrationState) {
  ...
  const campers = (state.formData.campers || []).map(camper =>
    jsonLogic.apply(pricingLogic.camper, {
      ...data,
      camper
    })
  );

  const meals = (state.formData.campers || []).map(camper =>
    jsonLogic.apply(pricingLogic.meals, {
      ...data,
      camper
    })
  );

  const parking = jsonLogic.apply(pricingLogic.parking, data);

  const shirts = jsonLogic.apply(pricingLogic.shirts, data);

  const donation = jsonLogic.apply(pricingLogic.donation, data);
  ...
  Object.entries({
    ...
    if (typeof value !== "number") {
      ...
      console.log(result);
      throw new Error(
        `Pricing returned non-natural number for ${key} (got ${value}).`
      );
    }
  });
